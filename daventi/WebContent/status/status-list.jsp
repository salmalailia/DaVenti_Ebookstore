...
<body>
	...
	<div class="row">
		<div class="container">
			<h3 class="text-center">List of Status</h3>
			<hr>
			<div class="container text-left">

				<a href="<%=request.getContextPath()%>/status/status-form.jsp" class="btn btn-success">Add
					New Status</a>
			</div>
			<br>
			<table class="table table-bordered">
				...
				<tbody>
					<c:forEach var="status" items="${listStatus}">
						<tr>
							<td><c:out value="${status.id_status}" /></td>
							<td><c:out value="${status.name_status}" /></td>
							<td><a class="btn btn-warning" href="?action=EDIT&id_status=<c:out value='${status.id_status}' />">Edit</a>
								&nbsp;&nbsp;&nbsp;&nbsp; 
								<a class="btn btn-danger" href="?action=DELETE&id_status=<c:out value='${status.id_status}' />">Delete</a>
							</td>
						</tr>
					</c:forEach>
				</tbody>
